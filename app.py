...
app.config["SQLALCHEMY_DATABASE_URI"] = db_url or "sqlite:///shop.db"
...
def init_db():
    with app.app_context():
        ...
        add_col("ALTER TABLE orders ADD COLUMN phone TEXT")
        add_col("ALTER TABLE orders ADD COLUMN address TEXT")
        add_col("ALTER TABLE orders ADD COLUMN status TEXT NOT NULL DEFAULT 'NEW'")
...
def checkout():
    ...
    # Postgres supports RETURNING. SQLite may not (depends on version). We'll handle both.
    order_id = None
    try:
        res = db.session.execute(text("""
            INSERT INTO orders (customer, phone, address, items, total, created_at, status)
            VALUES (:customer, :phone, :address, :items, :total, :created_at, :status)
            RETURNING id
        """), {
            "customer": customer,
            "phone": phone,
            "address": address,
            "items": json.dumps(items_for_db, ensure_ascii=False),
            "total": total,
...
def orders_page():
    rows = db.session.execute(text("""
        SELECT id, customer, phone, address, items, total, created_at, status
        FROM orders
        ORDER BY id DESC
    """)).mappings().all()
    ...
    for r in rows:
        orders.append({
            ...
            "order_items": json.loads(r["items"]) if r["items"] else [],
...
def update_order_status(order_id: int):
    new_status = (request.form.get("status") or "NEW").strip().upper()
